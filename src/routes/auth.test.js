...
test('GET /auth/verify, throws 422 when querystring is invalid', async () => {
  await request(app.listen()).get('/api/v1/auth/verify?token=test').expect(422);
});
...
test('GET /auth/reset, throws 422 when querystring is invalid', async () => {
  await request(app.listen()).get('/api/v1/auth/reset?token=test').expect(422);
});
...
test('POST /auth/reset, throws 422 when input is invalid', async () => {
  await request(app.listen()).post('/api/v1/auth/reset').send({ token: 'a' }).expect(422);
});
